...
const OddsCalculator = () => {
  ...
  const calculate = () => {
    ...
    setResult({ impliedProb: impliedProb.toFixed(1), edge: edge.toFixed(1), ev: (ev * 100).toFixed(1), isValue });
  };

  const reset = () => { setAiProb(''); setBookOdds(''); setResult(null); };

  return (
    <div style={{ background: '#0d1b2e', minHeight: '100vh', padding: '20px', fontFamily: 'monospace' }}>
      <div style={{ maxWidth: '600px', margin: '0 auto' }}>

        {/* Header */}
        <div style={{ marginBottom: '30px' }}>
          <h2 style={{ color: '#C8961E', margin: 0, fontSize: '20px', letterSpacing: '2px' }}>
            ODDS VALUE CALCULATOR
          </h2>
          <p style={{ color: '#5a7a9a', fontSize: '12px', margin: '6px 0 0' }}>
            Compare AI probability vs bookmaker implied odds
          </p>
        </div>
        ...
        <div style={{ marginBottom: '20px' }}>
          <label style={{ color: '#8899aa', fontSize: '11px', letterSpacing: '1px', display: 'block', marginBottom: '8px' }}>
            OUR AI WIN PROBABILITY (%)
          </label>
          <input
            type="number" min="0" max="100" value={aiProb}
            onChange={e => setAiProb(e.target.value)} placeholder="e.g. 73"
            style={{
              width: '100%', padding: '14px 16px', background: '#1a2940', border: '1px solid #2a3f5f',
              borderRadius: '8px', color: '#00d4ff', fontSize: '24px', fontFamily: 'monospace',
        ...
        <div style={{ marginBottom: '28px' }}>
          <label style={{ color: '#8899aa', fontSize: '11px', letterSpacing: '1px', display: 'block', marginBottom: '8px' }}>
            {bookmaker.toUpperCase()} DECIMAL ODDS
          </label>
          <input
            type="number" min="1" step="0.01" value={bookOdds}
            onChange={e => setBookOdds(e.target.value)} placeholder="e.g. 1.45"
            style={{
              width: '100%', padding: '14px 16px', background: '#1a2940', border: '1px solid #2a3f5f',
              borderRadius: '8px', color: '#C8961E', fontSize: '24px', fontFamily: 'monospace',
              outline: 'none', boxSizing: 'border-box'
            }}
          />
          <p style={{ color: '#5a7a9a', fontSize: '11px', margin: '6px 0 0' }}>
            Decimal odds format (1.45 = 45% implied probability)
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: '12px', marginBottom: '28px' }}>
          <button onClick={calculate} style={{
            flex: 1, padding: '14px', background: '#C8961E', border: 'none', borderRadius: '8px',
            color: '#0d1b2e', fontSize: '14px', fontWeight: 'bold', cursor: 'pointer', letterSpacing: '1px'
          }}>
            CALCULATE VALUE
          </button>
          <button onClick={reset} style={{
            padding: '14px 20px', background: '#1a2940', border: '1px solid #2a3f5f', borderRadius: '8px',
        ...
        {result && (
          <div style={{
            padding: '24px', borderRadius: '12px', border: `2px solid ${result.isValue ? '#00c853' : '#ff3d00'}`,
            background: result.isValue ? 'rgba(0,200,83,0.08)' : 'rgba(255,61,0,0.08)', marginBottom: '24px'
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
              <span style={{ fontSize: '32px' }}>{result.isValue ? '✅' : '❌'}</span>
              <div>
                <div style={{
                  color: result.isValue ? '#00c853' : '#ff3d00',
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
              ...
              <div style={{ background: '#0d1b2e', padding: '16px', borderRadius: '8px', textAlign: 'center' }}>
                <div style={{ color: result.isValue ? '#00c853' : '#ff3d00', fontSize: '24px', fontWeight: 'bold' }}>
            ...
            <div style={{ marginTop: '16px', padding: '14px', background: '#0d1b2e', borderRadius: '8px', textAlign: 'center' }}>
              ...
              <span style={{ color: result.isValue ? '#00c853' : '#ff3d00', fontSize: '18px', fontWeight: 'bold' }}>
        ...
        <div style={{ padding: '20px', background: '#1a2940', borderRadius: '10px', border: '1px solid #2a3f5f' }}>
          ...
          <div style={{ color: '#8899aa', fontSize: '12px', lineHeight: '1.8' }}>
            ...
            2. Check the same match odds on Bet365 / Betway<br/>
