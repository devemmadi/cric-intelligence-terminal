...
function LiveScorecard({ batters, bowler }) {
    ...
    return (
        <div style={{ background: "rgba(15,23,42,0.6)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "14px 16px", marginBottom: 14 }}>
            ...
            <div style={{ marginBottom: batters.length > 0 ? 10 : 0 }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 32px 32px 52px", gap: 4, marginBottom: 5, padding: "0 2px" }}>
                ...
                {batters.map((b, i) => (
                    <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 32px 32px 52px", gap: 4, padding: "5px 2px", borderTop: "1px solid rgba(255,255,255,0.05)", alignItems: "center" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                            {b.isStriker && <span style={{ width: 5, height: 5, borderRadius: "50%", background: "#22C55E", display: "inline-block", flexShrink: 0 }} />}
                            <span style={{ fontSize: 11, fontWeight: b.isStriker ? 700 : 400, color: b.isStriker ? "#E2E8F0" : "#94A3B8", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{b.name}</span>
                        </div>
                        ...
                        <span style={{ fontSize: 11, fontWeight: 600, textAlign: "right", color: b.sr >= 150 ? "#22C55E" : b.sr >= 100 ? "#F59E0B" : "#EF4444" }}>{b.sr ? b.sr.toFixed(0) : "0"}</span>
                    </div>
                ))}
            </div>
            {bowler && bowler.name && (
                <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: 8 }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 30px 30px 28px 44px", gap: 4, marginBottom: 4, padding: "0 2px" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 30px 30px 28px 44px", gap: 4, padding: "5px 2px", borderTop: "1px solid rgba(255,255,255,0.05)", alignItems: "center" }}>
                        <span style={{ fontSize: 11, fontWeight: 600, color: "#E2E8F0", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{bowler.name}</span>
...
{/* Row 4: Pitch + Weather + Wear inline */}
                                    ...
                                    <div style={{ position: "sticky", top: 80, display: "flex", flexDirection: "column", gap: 14 }}>
                                        {pred && pred.batters && pred.batters.length > 0 && (
                                            <div style={{ background: "rgba(15,23,42,0.5)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "12px 14px", marginBottom: 12 }}>
                                                <div style={{ fontSize: 9, fontWeight: 700, color: "#64748B", letterSpacing: 1.5, marginBottom: 8 }}>⚡ LIVE SCORECARD</div>
                                                <div style={{ display: "grid", gridTemplateColumns: "1fr 28px 28px 44px", gap: 2, marginBottom: 4 }}>
                                                    <span style={{ fontSize: 9, color: "#475569" }}>BATTER</span>
                                                    <span style={{ fontSize: 9, color: "#475569", textAlign: "right" }}>R</span>
                                                    <span style={{ fontSize: 9, color: "#475569", textAlign: "right" }}>B</span>
                                                    <span style={{ fontSize: 9, color: "#475569", textAlign: "right" }}>SR</span>
                                                </div>
                                                {pred.batters.map((b, i) => (
                                                    <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 28px 28px 44px", gap: 2, padding: "4px 0", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                                                        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                                                            {b.isStriker && <div style={{ width: 4, height: 4, borderRadius: "50%", background: "#22C55E", flexShrink: 0 }} />}
                                                            <span style={{ fontSize: 10, fontWeight: b.isStriker ? 700 : 400, color: b.isStriker ? "#E2E8F0" : "#94A3B8", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{b.name}</span>
                                                        </div>
                                                        <span style={{ fontSize: 11, fontWeight: 700, color: "#E2E8F0", textAlign: "right" }}>{b.runs}</span>
                                                        <span style={{ fontSize: 10, color: "#94A3B8", textAlign: "right" }}>{b.balls}</span>
                                                        <span style={{ fontSize: 10, fontWeight: 600, color: b.sr >= 150 ? "#22C55E" : b.sr >= 100 ? "#F59E0B" : "#EF4444", textAlign: "right" }}>{b.sr ? b.sr.toFixed(0) : "0"}</span>
                                                    </div>
                                                ))}
                                                {pred.bowler && pred.bowler.name && (
                                                    <div>
                                                        <div style={{ display: "grid", gridTemplateColumns: "1fr 28px 28px 24px 38px", gap: 2, marginTop: 8, marginBottom: 4, borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: 6 }}>
                                                            <span style={{ fontSize: 9, color: "#475569" }}>BOWLER</span>
                                                            <span style={{ fontSize: 9, color: "#475569", textAlign: "right" }}>O</span>
                                                            <span style={{ fontSize: 9, color: "#475569", textAlign: "right" }}>R</span>
                                                            <span style={{ fontSize: 9, color: "#475569", textAlign: "right" }}>W</span>
                                                            <span style={{ fontSize: 9, color: "#475569", textAlign: "right" }}>ECO</span>
                                                        </div>
                                                        <div style={{ display: "grid", gridTemplateColumns: "1fr 28px 28px 24px 38px", gap: 2, padding: "4px 0", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                                                            <span style={{ fontSize: 10, fontWeight: 600, color: "#E2E8F0", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{pred.bowler.name}</span>
                                                            <span style={{ fontSize: 10, color: "#94A3B8", textAlign: "right" }}>{pred.bowler.overs}</span>
                                                            <span style={{ fontSize: 10, color: "#94A3B8", textAlign: "right" }}>{pred.bowler.runs}</span>
                                                            <span style={{ fontSize: 11, fontWeight: 700, color: pred.bowler.wickets > 0 ? "#22C55E" : "#E2E8F0", textAlign: "right" }}>{pred.bowler.wickets}</span>
                                                            <span style={{ fontSize: 10, fontWeight: 600, color: pred.bowler.economy <= 7 ? "#22C55E" : pred.bowler.economy <= 10 ? "#F59E0B" : "#EF4444", textAlign: "right" }}>{pred.bowler.economy ? pred.bowler.economy.toFixed(1) : "0.0"}</span>
                                                        </div>
                                                    </div>
                                                )}
                                            </div>
                                        )}
                                        {pred && pred.batters && pred.batters.length > 0 && <LiveScorecard batters={pred.batters} bowler={pred.bowler || {}} />}
