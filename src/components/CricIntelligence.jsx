...
function LiveScorecard({ batters, bowler }) {
    ...
    return (
        <div style={{ background: "rgba(15,23,42,0.6)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "14px 16px", marginBottom: 14 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: "#94A3B8", letterSpacing: 1.5, marginBottom: 10 }}>{"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¡ LIVE SCORECARD"}</div>
            <div style={{ marginBottom: 10 }}>
...
WEATHER: ${pred.weather?.condition||""} ${pred.weather?.temp||""}ÃÂ°C Humidity:${pred.weather?.humidity||""}%
AT CREASE: ${batters}
BOWLING: ${bowler}
...
Give me:
1. **WIN PREDICTION** Ã¢ÂÂ who wins and why (be confident, give %)
2. **NEXT 5 OVERS** Ã¢ÂÂ exact runs range expected, wicket risk
3. **GAME-CHANGER** Ã¢ÂÂ one factor that will decide this match
4. **STRATEGY CALL** Ã¢ÂÂ what should batting/bowling team do RIGHT NOW
        ...
        <div className="card" style={{ margin: "0 20px 16px", padding: 20, border: "1px solid rgba(139,92,246,0.3)", background: "linear-gradient(135deg,rgba(139,92,246,0.05),rgba(99,102,241,0.05))" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                ...
                <button onClick={askClaude} disabled={loading} style={{ background: loading ? "#334155" : "linear-gradient(135deg,#7C3AED,#6366f1)", border: "none", color: "#fff", padding: "8px 18px", borderRadius: 8, cursor: loading ? "not-allowed" : "pointer", fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}>
                    {loading ? <><span style={{ display: "inline-block", width: 10, height: 10, border: "2px solid #fff", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }}></span> Analyzing...</> : asked ? "Ã¢ÂÂ» Refresh" : "Ã¢ÂÂ¡ Get AI Analysis"}
                </button>
            </div>
            {!asked && !loading && (
                <div style={{ textAlign: "center", padding: "20px 0", color: "#64748B", fontSize: 13 }}>
                    Click <strong style={{color:"#a78bfa"}}>Get AI Analysis</strong> Ã¢ÂÂ Claude will analyze live match data and give you sharp predictions
                </div>
            )}
            {loading && (
