...
export default function CricIntelligence() {
    ...
    const fetchMatches = useCallback(async () => {
        try {
          ...
          const firstLive = data.find(x => x.liveData);
          if (firstLive) setSelectedMatch(firstLive.id); setLiveStatus("live");
                        ...
                        {liveMatches.map(m => (
                            <div key={m.id} className={`match-pill ${selectedMatch.id===m.id?"sel":""}`} onClick={() => setSelectedMatch(m)}>
                                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                                    ...
                                    <span style={{ fontSize: 9, fontWeight: 700, padding: "1px 6px", borderRadius: 5, background: (m.liveData||m.status==="LIVE")?"#FFF0F0":C.bg, color: (m.liveData||m.status==="LIVE")?C.red:C.muted }}>
                                        {(m.liveData||m.status==="LIVE")?"LIVE":"UPCOMING"}
                                    </span>
                                </div>
